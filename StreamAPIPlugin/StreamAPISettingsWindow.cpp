...
std::vector<ExternalConsoleCommand> externalConsoleCommands({
	...
	ExternalConsoleCommand{ -1, "", true, "", "cl_itemmod_<enabled/code>: use \"cl_itemmod_enabled 1; cl_itemmod_code ${code};\" to allow viewers to equip items with item mod codes by exposing the code parameter through your bot command. I have plans to release a webpage soon that allows viewers to create designs in browser and generate the item mod code. There is a small issue when it comes to applying paints to different car bodies than what's currently equipped. I will be looking into talking to BM devs and possible Psyonix devs about adding the ability to change the car body the next time you load into the main menu." },
